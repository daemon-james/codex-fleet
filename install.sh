...
#   ~/.claude/skills/codex-fleet/      Claude Code discovers the skill here
#
...
link "$ROOT/skills/codex-fleet/SKILL.md"  "$HOME/.claude/skills/codex-fleet/SKILL.md" "claude skill" || rc=1
...
if (( ! CHECK )); then mkdir -p "$HOME/.codex-fleet"; fi
...
echo
echo "Next: let Claude Code follow SETUP.md to merge hook registration:"
...
echo "Skill destination: ~/.claude/skills/codex-fleet/SKILL.md"
echo "See SETUP.md for setup verification and any required trust prompts."
exit $rc
